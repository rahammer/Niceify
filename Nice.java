...
public class Nice extends JPanel{
	...
	/**
	 * Nice - Takes in a width and a height and creates
	 * a Panel with a button three labels, and two textfields
	 *
	 * @param width
	 * @param height
	 */
	...
	/**
	 * makeNice - takes in the string and replaces
	 * all the negative words with positive words
	 * and then returns the positive statement as
	 * a string.
	 * 
	 * @param meanString
	 * @return result
	...
	private String makeNice(String meanString){
		...
		//load badwords from list and splits niceified sentence into individual words to be analyzed
		BadWords b = new BadWords();
		ArrayList<String> badwords = b.badList();
		String sentence[] = result.split(" ");
		
		//find bad words in niceified sentence
		for(int i = 0; i < sentence.length; i++){
			for(int j = 0; j < badwords.size(); j++){
				if(sentence[i].trim().equalsIgnoreCase(badwords.get(j).trim())){
					result = "Watch your profanity";
				}
			}
		}
		return result;
